...
void RestPlugin::OnNotification(const Notification& notif) {
	...
	// if any plugin disconnects from rest plugin, we need to remove all related rest commands.
...
void RestPlugin::ParseDataReceived(void* buffer, unsigned int nbytes, ConnectionHandle connection, PluginHandle plugin) {
	JsonReader reader;
	...
	if(reader.Parse((const char*)buffer, nbytes)) {
		...
		if(type == "register_command") {
			...
			PLUGIN_INFO("received register request for %s. from plugin: %p.", command_name.c_str(), plugin);
			...
			PLUGIN_INFO("received response data for request %d. from plugin %p.", request_id, plugin);
			OnResponseData(request_id, response_data.c_str(), (int)response_data.length(), connection);
		} else if(type == "unregister_command") {
			int command_id;
			if(reader.Read("command_id", &command_id)) {
				PLUGIN_INFO("receive unregister request for command id %d. from plugin %p.", command_id, plugin);
...
void RestPlugin::OnRegisterCommand(const char* command_name, ConnectionHandle connection, PluginHandle plugin) {
	std::string output;
	...
	PLUGIN_INFO("registering command %s for plugin %s(%s).", command_name, info.name, info.version);

	writer.Write<const char*>("command_name", command_name);

	std::string command_long = ok ? GetFullCommandName(command_name, info) : command_name;
	...
	PLUGIN_INFO("sending %s registration response for %s", ok ? "success" : "failure", ok ? command_long.c_str() : command_name);
...
void RestPlugin::OnUnregisterCommand(int command_id, ConnectionHandle connection) {
	(void)connection;
	m_server.Unregister(command_id);
}

void RestPlugin::OnResponseData(int request_id, const char* response_data, int response_length, ConnectionHandle connection) {
	PLUGIN_INFO("sending http response id: %d. \"%s\"", request_id, response_data);
	m_server.SendResponse(connection, request_id, response_data, response_length);
}

std::string RestPlugin::GetFullCommandName(const char* cmd_name, const PluginInfo& info) {
	std::string result = "/";

	bool correct_plugin = Base::String::Compare(RestPlugin::Name, info.name)==0 ||
	...
	if(!correct_plugin) {
		...
		if(0 != Base::String::Compare(cmd_name, "command-list")) {
			result += info.hostname; result += "/";
			result += pid; result += "/";
			result += info.name; result += "/";
			result += info.version; result += "/";
		}
