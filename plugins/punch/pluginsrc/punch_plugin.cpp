...
PunchPlugin::PunchPlugin() : SimplePlugin(kUpdateDeltaMs) {
  m_recv_buffer = malloc(kRecvBufferSize);
}

PunchPlugin::~PunchPlugin() { free(m_recv_buffer); }
...
bool PunchPlugin::OnStartup(const char *config, streamsize nbytes) {
  ...
  PLUGIN_INFO("punch server started at: %s:%s", PRINTF_URL(url));
  return m_punch != nullptr;
}

void PunchPlugin::OnShutdown() { punch_server_destroy(m_punch); }
...
void PunchPlugin::ParseDataReceived(void *buffer, unsigned int nbytes,
