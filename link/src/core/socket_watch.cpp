...
bool SocketWatch::Register(Base::Socket::Handle socket,
                           ConnectionHandle handle) {
#if defined __APPLE__
  struct kevent event;

  // EFILT_READ - if listen() read is returned if there is a connection to
  // accept. data contains the size of backlog. EFILT_READ - other - data to
  // read. EFILT_WRITE - data may be written.

  EV_SET(&event,                     // event to initialize
         socket,                     // file to watch
...
bool SocketWatch::Wait(time_ms timeout,
                       std::function<void(ConnectionHandle connection,
                                          bool read, bool write, bool error)>
                           func) {
#if defined __APPLE__
  __darwin_time_t timeout_sec = timeout / 1000;
  long timeout_us = ((timeout - timeout_sec * 1000) * 1000000);
  struct timespec timeoutspec = {timeout_sec,
                                 timeout_us}; // seconds, nanoseconds.
  ...
  for(u16 i = 0; i <
                 ...
                 1;
      ++i) {
    // event to init, event id, kernel filter, action to perform on event,
    // filter spec flags, filter spec data, user data.

    // notify when data available to read. on return 'data' contains the number
    // of bytes ready for reading.
    // EV_SET(&monitor_list[i], m_sockets[i].GetHandle(), EVFILT_READ, EV_ADD,
    // 0, 0, nullptr);
  }

  // block until events available.
  signed int num_events =
      kevent(m_kqueue, nullptr, 0, response_list, max_events, &timeoutspec);
