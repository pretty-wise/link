...
public:
		...
		WatchHandle Create(const char* name_filter, const char* version_filter, const char* hostname_filter);

		// Removes watch by handle.
		void Remove(WatchHandle handle);

		// Returns a list of watches matching given name & version.
		void Filter(const char* name, const char* version, const char* hostname, std::vector<WatchHandle>& handles) const;
