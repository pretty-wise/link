...
private:
  ...
  void OnRegisterCommand(const char *command_name, ConnectionHandle connection,
                         PluginHandle plugin);
  void OnUnregisterCommand(int command_id, ConnectionHandle handle);
  void OnResponseData(int request_id, const char *response_data,
                      int response_length, ConnectionHandle handle);
...
SimplePlugin *SimplePlugin::CreatePlugin() { return new Link::RestPlugin(); }
