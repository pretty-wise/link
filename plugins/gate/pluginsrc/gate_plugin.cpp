...
void OnConnected(TCPServer::Handle handle, const Base::Url& address, void* context) {
	PLUGIN_INFO("connection established %d. ip %d.%d.%d.%d:%d", handle, PRINTF_URL(address));
	GatePlugin* this_ = static_cast<GatePlugin*>(context);
	this_->AddConnection(handle, address);
}

void OnDisconnected(TCPServer::Handle handle, TCPServer::CloseReason reason, const Base::Url& address, void* context) {
	PLUGIN_INFO("disconnected %d. reason %s. ip %d.%d.%d.%d:%d", handle, TCPServer::ToString(reason), PRINTF_URL(address));
	GatePlugin* this_ = static_cast<GatePlugin*>(context);
	this_->RemConnection(handle);
}
void OnMessage(TCPServer::Handle handle, void* buffer, u32 nbytes, void* context) {
...
bool GatePlugin::OnStartup(const char* config, streamsize nbytes) {	
	...
	if(!RegisterCommand(&m_list_cmd)
		|| !RegisterCommand(&m_disconnect_cmd)
...
void GatePlugin::ParseDataReceived(void* buffer, unsigned int nbytes, ConnectionHandle connection, PluginHandle plugin) {
}

void GatePlugin::AddConnection(TCPServer::Handle handle, const Base::Url& address) {
...
void GatePlugin::HandleMessage(TCPServer::Handle handle, void* data, u32 nbytes) {
	...
	switch(type) {
		case gate::ClientMessage::kLogin:
		{
			...
			if(!m_conn->Send(handle, static_cast<const void*>(serialized.c_str()), serialized.length())) {
				m_conn->Close(handle, TCPServer::CloseReason::kProtocolError); // todo(kstasik): new error type
			}
		}
		break;
		case gate::ClientMessage::kLogout:
		{
			...
			response.set_protocol_version(kProtocolVersion); // todo(kstasik): refactor protoc version
			...
			if(!m_users->RemUser(handle, logout.user_id())) {
				PLUGIN_ERROR("problem logging out handle: %d index %d", handle, logout.user_id());
...
bool GatePlugin::Logout(UserId id) {
	...
	std::string serialized;
	response.SerializeToString(&serialized);
	if(!m_conn->Send(conn.GetHandle(), static_cast<const void*>(serialized.c_str()), serialized.length())) {
		m_conn->Close(conn.GetHandle(), TCPServer::CloseReason::kWriteFailure); // todo(kstasik): new error type
		return false;
	}
	return true;
}

void GatePlugin::WriteUserList(std::string* data) {
	m_users->Write(data);
}
