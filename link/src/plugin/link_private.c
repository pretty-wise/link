...
int GetInterfaceVersion() {
  return LINK_INTERFACE_VERSION;
}

int StartupPlugin(struct LinkInterface* iface, const char* config, streamsize nbytes) {
    s_interface = *iface;
...
WatchHandle CreateWatch(const char* name_match, const char* version_match, const char* hostname_match) {
  return s_interface.CreatePluginWatch(s_interface.context, name_match ? name_match : "", version_match ? version_match : "", hostname_match ? hostname_match : "");
}
...
void LogWritePlugin(const char* file, int line, int category, const char* format, ...)  {
  va_list args;
  va_start(args, format);
  s_interface.LogWrite(s_interface.context,
                             file,
                             line,
                             category,
                             format,
