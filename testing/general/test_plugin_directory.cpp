...
TEST(PluginDirectory, Registration) {
  ...
  ASSERT_TRUE(handle_A ==
              dir.GenerateHandle(info_A)); // same handle generated twice.
  ...
  ASSERT_TRUE(handle_B ==
              dir.GenerateHandle(info_B)); // same handle generated twice.

  ASSERT_TRUE(
      !dir.Register(handle_A, info_B)); // register old handle with new info.

  result = dir.Register(handle_B, info_B);
  ASSERT_TRUE(result);
  ASSERT_TRUE(!dir.Register(handle_B, info_B)); // double re-registration.
  ASSERT_TRUE(
      !dir.Register(handle_B, info_A)); // double re-registration, diff info.
}

class AvailabilityListener : public Link::PluginDirectoryListener {
public:
  AvailabilityListener(PluginHandle plugin, PluginInfo info)
      : m_plugin(plugin), m_available(false), m_error(false) {
